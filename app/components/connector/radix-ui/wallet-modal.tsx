...
export function WalletModal({ open, onOpenChange }: WalletModalProps) {
    ...
    const LAZORKIT_WALLET_NAME = 'Lazorkit Wallet';

    const sortedInstalledWallets = [...installedWallets].sort((a, b) => {
        const aIsLazorKit = a.wallet.name === LAZORKIT_WALLET_NAME;
        const bIsLazorKit = b.wallet.name === LAZORKIT_WALLET_NAME;

        // 1️⃣ LazorKit always on top
        if (aIsLazorKit && !bIsLazorKit) return -1;
        if (!aIsLazorKit && bIsLazorKit) return 1;

        // 2️⃣ Recently connected next
        ...
        // 3️⃣ Keep original relative order
        return 0;
    });
